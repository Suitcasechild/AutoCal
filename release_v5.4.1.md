...
*   **Enhanced Reporting:** Report summary now consistently shows all calibration factors (V, A, W) for completeness, regardless of whether they were changed.
*   **GUI Stability & Safety:** Implemented comprehensive error handling (try-finally blocks) to ensure the interface remains responsive and the DUT is powered off even on transmission failures.
*   **Help Asset Completion:** The integrated manual now covers all technical and operational details directly within the help window.
*   **Fixes:** Resolved a crash in manual mode when devices were unreachable and fixed a regression button visibility issue in HOME/MANUAL modes.
...
*   **Robuste Kommunikation:** Enhanced Fluke 45 RS232 logic and refactored DUT initialization with proper URL encoding and full authentication support.
*   **Code Hardening:** Comprehensive audit of all modules for improved error handling and stability against edge cases.

**Note for Users:** Existing reports remain compatible. Internal versioning is synchronized to v5.4.1.

---

# 🚀 Tasmota Precision Calibrator v5.4.1 (DE)
...
*   **Verbessertes Protokoll:** Die Zusammenfassung am Ende zeigt nun zur Vollständigkeit immer alle Faktoren (V, A, W) an, auch wenn diese nicht geändert wurden.
*   **GUI-Stabilität & Sicherheit:** Umfassende Fehlerbehandlung (Try-Finally) stellt sicher, dass die GUI bedienbar bleibt und die Dose auch bei Übertragungsfehlern sicher abgeschaltet wird.
*   **Vollständige Hilfe-Assets:** Die integrierte Anleitung wurde vervollständigt und deckt nun alle technischen und operativen Details direkt im Hilfefenster ab.
*   **Fehlerbehebungen:** Absturz im manuellen Modus bei Nichterreichbarkeit der Dose behoben und Sichtbarkeit des Regressions-Buttons korrigiert.
...
*   **Robuste Kommunikation:** Verbesserte Fluke 45 RS232-Logik und überarbeitete Initialisierung der Dose mit korrekter URL-Kodierung und vollem Auth-Support.
*   **Code-Härtung:** Umfassende Überprüfung aller Module für bessere Fehlerbehandlung und Stabilität in Randfällen.

**Hinweis für Anwender:** Bestehende Kalibrier-Reports bleiben kompatibel. Alle internen Versionsangaben wurden auf v5.4.1 synchronisiert.
