...
*   **Integrierte interaktive Anleitung:**
    Unter **Hilfe -> Anleitung** steht nun eine extrem detaillierte Bedienungsanleitung zur Verfügung. Diese ist fest in die Software eingebettet (Schutz vor Manipulation) und bietet ein interaktives Inhaltsverzeichnis mit Anker-Navigation. Die Anleitung wurde in einem hochwertigen Dark-Theme gestaltet und enthält Schritt-für-Schritt-Anweisungen mit technischen Hintergründen (z.B. Warum Cal-Befehle?).
    *   **English:** Integrated Interactive Manual: An extremely detailed instruction manual is now available under **Help -> Instructions**. It is embedded directly into the software (protection against manipulation) and features an interactive table of contents with anchor navigation. The manual is designed in a high-quality dark theme and contains step-by-step instructions with technical backgrounds.
