...
*   **Multi-Punkt-Messung:** Die Messung erfolgt über eine in der GUI definierte Anzahl von Stufen und Dauer. Die Software zeichnet für jede Stufe eine Serie von Messwert-Paaren (Referenz vs. Prüfling) auf und speichert sie in einer separaten `.csv`-Datei.
...
*   **Live-GUI:** Die Benutzeroberfläche, erstellt mit `PySide6`, zeigt Live-Daten in `pyqtgraph`-Diagrammen an. Ein Log-Fenster informiert über den Programmablauf.
