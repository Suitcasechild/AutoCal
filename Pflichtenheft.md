...
### 2.6 Authentifizierung (Passwortschutz)
*   **Dynamische Passwortabfrage:** Erkennt die Software bei einem Verbindungsversuch (sowohl beim "Online Check" als auch während des Messprozesses) einen `401 Unauthorized`-Fehler, wird der Nutzer automatisch durch einen Dialog zur Eingabe von Benutzername und Passwort für das betreffende Gerät aufgefordert.
*   **Wiederholungslogik:** Die Eingabe kann bei einem Fehler bis zu zwei Mal wiederholt werden (insgesamt drei Versuche). Bricht der Nutzer den Dialog ab, wird der gesamte Prozess gestoppt.
*   **Temporäre Speicherung:** Die eingegebenen Zugangsdaten werden nur für die Dauer des aktuellen Kalibrierprozesses im Arbeitsspeicher gehalten und danach verworfen. Sie werden **niemals** auf die Festplatte geschrieben.
...
*   **Code-Dokumentation:** Der gesamte Quellcode wird gemäß Projektstandard mit Docstrings und zweisprachigen (Englisch/Deutsch) Kommentaren versehen, um die Lesbarkeit und Wartbarkeit zu maximieren.
